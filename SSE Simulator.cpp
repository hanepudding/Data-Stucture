...
public:
	...
	double turnover;
	//初始化 
	lib(int no,int d,string BOrS,int code,double At,int Lots){
		num=no;
		day=d;
		BuyOrSell=BOrS;
		BuyOrSellStock=code;
		BuyOrSellAt=At;
		BuyOrSellLots=Lots;
		turnover=BuyOrSellAt*BuyOrSellLots;
	}
	lib(){
		;
	}
...
int main(){
    ...
    fout<<"\t\t\t本次模拟记录\nNo.\tDAY\tB/S\tStock Code\tPrice\tLots\t\tTurnover\n";
    fout.close();
	//初始化一个股票账户 
	account Harune;
	//用户界面 
			...
			if(cmd == 1){
				...
				if(Harune.balance==0)
					...
					if(bullish < 0 or bullish > Harune.balance / SSE.get(code)){
						...
						Harune.show();
						//创建记录并入队 
						lib temp(number,i,"Buy",code,SSE.get(code),bullish);
						record.in(temp);
						number++;
					}
				}
				...
				if(bearish<0){
					...
					Harune.show();
					//创建记录并入队 
					lib temp(number,i,"Sell",code,SSE.get(code),bearish);
					record.in(temp);
					number++;
				}
			}else if(cmd ==  3){
	...
	while(not record.isEmpty()){
		...
		temp=record.out();
		counter++;
		t.set(temp.num,temp.turnover);
		fout1<<fixed << setprecision(2)<<temp.num<<"\t"<<temp.day<<"\t"<<temp.BuyOrSell<<"\t"<<temp.BuyOrSellStock<<"\t\t"<<temp.BuyOrSellAt<<"\t"<<temp.BuyOrSellLots<<"\t\t"<<temp.turnover<<endl;
	}
	
	fout1<<"\n\n\n\t"<<"成交金额最高的5笔交易\n"; 
	fout1<<"Rank\tTrade No.\tTurnover\n";
	double MAX=0;
	int pin=0;
	
	int dotime=0;
	while(dotime<5 and dotime<counter){
		...
		dotime++;
		fout1<<dotime<<"\t"<<pin<<"\t\t"<<t.get(pin)<<endl;
		t.set(pin,0);
		MAX=0;
	}
